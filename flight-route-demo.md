---
layout: default
title: 3D Flight Route Demo - SF to London
---
...
<head>
  <meta charset="utf-8">
  <title>3D Flight Route Demo - SF to London</title>
...
<body>
  ...
  <script>
    ...
    // Map style with sources and layers
    const style = {
      'version': 8,
      'imports': [
        {
    ...
    // Update model source and feature state
    ...
    // Animation loop
    map.on('load', () => {
      const animationDuration = 20000; // 4x faster than original (was 50000)
      const flightTravelAltitudeMin = 200; // Increased from 200 to 500 - speeds up low altitude phase
      const flightTravelAltitudeMax = 1500; // Reduced from 3000 - reaches max zoom faster
      ...
      function frame(time) {
        ...
        const timelapseFactor = mix(0.002, 6.0, animFade * animFade);
