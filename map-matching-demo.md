...
<body>
  ...
  <script>
    ...
    function splitCoordinatesByGaps(points, gapThresholdMeters = 50) {
      ...
      for (let i = 1; i < points.length; i++) {
        ...
        if (distance > gapThresholdMeters) {
          ...
          console.log(`GPS gap detected: ${distance.toFixed(1)}m between point ${i-1} and ${i}`);
        } else {
      ...
      if (currentSegment.length >= 2) {
        ...
        // If last segment has only 1 point, discard it
        console.log('Last segment has only 1 point, discarding it');
      }

      console.log(`Split ${points.length} points into ${segments.length} segments`);
      segments.forEach((seg, idx) => {
        console.log(`  Segment ${idx + 1}: ${seg.length} points`);
      });
    ...
    async function matchRoute() {
      ...
      try {
        // Split points into segments based on GPS gaps (>50m)
        ...
        for (let i = 0; i < segments.length; i++) {
          ...
          updateStatus('loading', `セグメント ${i + 1}/${segments.length} を処理中...`,
            `${segment.length} 地点を道路にマッチングしています...`);

          // Clean anomalies from segment
          const cleanedSegment = cleanPointAnomalies(segment);

          if (cleanedSegment.length < 2) {
            console.log(`Segment ${i + 1} has less than 2 points after cleaning, skipping`);
            failedSegments++;
            continue;
          }
          ...
          try {
            ...
            if (data.matchings && data.matchings.length > 0) {
              ...
              console.log(`No matching found for segment ${i + 1}`);
              failedSegments++;
            }
          } catch (segmentError) {
            console.error(`Error matching segment ${i + 1}:`, segmentError);
            failedSegments++;
          }
        }
