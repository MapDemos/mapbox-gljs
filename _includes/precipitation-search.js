...
// Get precipitation value at a specific coordinate using Tilequery API
async function getPrecipitationAtLocation(lng, lat, bandIndex = null, retryCount = 0) {
    try {
        ...
        // Get the tileset ID from the source configuration
        const tilesetConfig = tilesets[bandInfo.tilesetId];
        const tilesetId = tilesetConfig.value.replace('mapbox://', '');
        const bandString = String(bandInfo.band);

        // Construct Tilequery API URL
        // Format: https://api.mapbox.com/v4/{tileset_id}/tilequery/{lng},{lat}.json
        // The layers parameter specifies which raster layer to query
        const url = `https://api.mapbox.com/v4/${tilesetId}/tilequery/${lng},${lat}.json?access_token=${mapboxgl.accessToken}&layers=${bandInfo.layer}`;
        ...
        // Parse the Tilequery API response
        // The API returns all bands at the location, we need to find the matching band
        if (data.features && data.features.length > 0) {
            // Find the feature that matches our target band
            const matchingFeature = data.features.find(feature => {
                return feature.properties?.tilequery?.band === bandString;
            });

            if (matchingFeature && matchingFeature.properties?.val) {
                const value = matchingFeature.properties.val[0];
                return value;
            }
        }

        // If no data found and we haven't retried too many times, retry after a delay
        // Increase retry count to 5 and longer delays for tile loading
        if (retryCount < 5) {
            const delay = 500 * (retryCount + 1); // 500ms, 1000ms, 1500ms, 2000ms, 2500ms
            await new Promise(resolve => setTimeout(resolve, delay));
            return await getPrecipitationAtLocation(lng, lat, bandIndex, retryCount + 1);
        }

        return null;
    } catch (error) {
        console.error('Error querying via Tilequery API:', error);
        return null;
    }
}
