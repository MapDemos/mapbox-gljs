// Default coordinates - Auckland, New Zealand
const defaultCoordinates = [174.763, -36.848];
...
const loadMap = () => {
    ...
    map.on('load', () => {
        ...
        map.addSource('naurt-doors', {
            type: 'vector',
            url: 'mapbox://kenji-shima.naurt-doors-nz'
        });

        // Add Naurt Addresses tileset source
        map.addSource('naurt-addresses', {
            type: 'vector',
            url: 'mapbox://kenji-shima.naurt-addresses-nz'
        });
        ...
        map.on('click', 'doors-layer', async (e) => {
            ...
            try {
                const response = await fetch(
                    `https://api.mapbox.com/search/geocode/v6/reverse?longitude=${lng}&latitude=${lat}&language=en&access_token=${mapboxgl.accessToken}`
                );
