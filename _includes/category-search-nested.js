...
// Data structures for merchant layer details (same as merchants-overlay-demo)
...
function clearResults() {
    ...
    // Show the merchants POI layer back when clearing results (including when switching search modes)
    if (map.getLayer('merchants')) {
        map.setLayoutProperty('merchants', 'visibility', 'visible');
    }

    // Reset global results arrays
    currentResults = [];
...
async function performSuggestSearch() {
    ...
    try {
        ...
        if (features.length > 0) {
            // Hide the merchants POI layer when showing search results
            if (map.getLayer('merchants')) {
...
function clearMarkers() {
    ...
    // Show the merchants POI layer back when clearing search results
    if (map.getLayer('merchants')) {
        map.setLayoutProperty('merchants', 'visibility', 'visible');
    }
}
...
map.on('load', () => {
    ...
    map.on('mouseleave', 'merchants', () => {
        map.getCanvas().style.cursor = '';
    });

    // Add click handler for merchants layer POIs
    map.on('click', 'merchants', (e) => {
        ...
        const categoryName = MERCHANT_CATEGORIES[categoryId] ? MERCHANT_CATEGORIES[categoryId].ja : `カテゴリー ${categoryId}`;
        const genreName = MERCHANT_GENRES[genreId] || `ジャンル ${genreId}`;
        const brandName = MERCHANT_BRANDS[brandId] || 'なし';

        // Create popup content with Japanese labels
        let popupContent = `<div style="padding: 8px;">
          <strong>店舗名:</strong> ${properties.n || '不明'}<br>
    ...
    map.on('click', (e) => {
        ...
        // Check if clicking on a merchant POI
        let clickedOnMerchant = false;
        if (map.getLayer('merchants')) {
            const merchantFeatures = map.queryRenderedFeatures(e.point, {
                layers: ['merchants']
            });
            clickedOnMerchant = merchantFeatures.length > 0;
        }

        // If not clicking on a result marker or merchant POI, set the new center
        if (!clickedOnResult && !clickedOnMerchant) {
