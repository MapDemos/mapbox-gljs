...
- **City agreement is very high** (~98%). The primary failure cases are suburb vs. sub-locality naming — for example, Mapbox returns "North Billerica" for coordinates that Naurt labels as "Billerica, Town Of". These are valid addresses in the same municipality but use finer-grained place names.
- **Street name agreement is strong** (~91%), with failures driven by abbreviation variations (e.g. `Mt` → `Mount`), differing street type classification (e.g. Naurt `Keneson Street` vs Mapbox `Keneson Road`), and a small number of complete mismatches where Mapbox snaps to a nearby street.
- **Street number match is the lowest metric** (~78% addresses, ~75% doors). Mapbox commonly returns an interpolated range rather than the exact parcel number Naurt records. Excluding street number raises the match rate to ~90%.
