...
- **The gap between `num_match` (85.6%) and `Num + Street + City` (72.7%) is driven by 516 records** where the street number is right but street name or city fails. Breaking this down:
  - **Macron/diacritic mismatch (136 cases, recoverable)** — Mapbox uses macrons in te reo Māori place names (`Kumeū`, `Waekāhu`) while Naurt does not. Adding Unicode diacritic stripping (NFD normalisation) to the pipeline would recover these without masking real differences.
  - **`Clos` abbreviation gap (13 cases, recoverable)** — `Clos` is not expanded to `Close` in the current normalizer. Adding this abbreviation would recover these cases.
  - **Street type disagreement (185 cases, genuine)** — Naurt and Mapbox occasionally classify the same road with a different type (e.g. Naurt `Burtons Rd`, Mapbox `Burtons Drive`). This is a data difference, not a formatting issue, and cannot be resolved by normalization without masking real mismatches.
  - **City sub-locality splits (182 cases, genuine)** — similar to the locality issue above; Mapbox resolves to a finer-grained or differently-bounded place name.
